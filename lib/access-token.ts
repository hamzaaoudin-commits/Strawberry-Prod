/**
 * Signature d'accès — cookies signés côté serveur.
 *
 * Extrait de l'ancien `lib/radar-access.ts` lors du retrait de RADAR : la
 * mécanique de signature ne concernait pas RADAR en propre, elle est utilisée
 * par la remise de l'Atlas (cookie posé après capture de l'e-mail dans
 * `app/api/contact/route.ts`). Supprimer le fichier d'origine sans extraire
 * ces fonctions aurait cassé l'Atlas et le point d'entrée du formulaire de
 * contact.
 *
 * Ce que le navigateur ne peut pas faire : fabriquer ce cookie. Le secret
 * n'est jamais comparé côté client et n'apparaît jamais dans le bundle.
 */
